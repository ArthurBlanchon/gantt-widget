...
import { computeSequenceSubRowCount } from "@/lib/gantt-row-layout";
...
const weeksInMonth = (year: number, monthIndex: number): number => {
...
const weekStartInMonth = (
  ...
  return addWeeks(firstWeekStart, weekIndex);
};
...
const getTimelineStartDate = (
  ...
  const year = timelineData.at(0)?.year ?? 0;
  const jan1 = new Date(year, 0, 1);

  if (range === "weekly") {
    return startOfWeek(jan1, weekStartsOn);
  }
...
/**
 * Weekly headers list one column per week-start that falls in each month, so the
 * same week can appear twice at month boundaries. Offsets must walk that same
 * column sequence — not a flat differenceInWeeks from timeline start.
 */
const getWeeklyHeaderColumnOffset = (
  date: Date,
  timelineData: TimelineData,
  timelineStartDate: Date
): { weekColumns: number; dayInWeek: number } | null => {
  const targetWeekStart = startOfWeek(date, weekStartsOn);
  let weekColumns = 0;

  for (const yearObj of timelineData) {
    for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
      const monthStart = startOfMonth(new Date(yearObj.year, monthIndex, 1));
      const monthEnd = endOfMonth(monthStart);
      let weekStart = startOfWeek(monthStart, weekStartsOn);
      const lastWeekStart = startOfWeek(monthEnd, weekStartsOn);

      while (weekStart <= lastWeekStart) {
        if (weekStart < timelineStartDate) {
          weekStart = addWeeks(weekStart, 1);
          continue;
        }
        if (weekStart.getTime() === targetWeekStart.getTime()) {
          return {
            weekColumns,
            dayInWeek: differenceInDays(startOfDay(date), weekStart),
          };
        }
...
const getDateFromWeeklyHeaderColumn = (
  mouseX: number,
  timelineData: TimelineData,
  timelineStartDate: Date,
  columnWidth: number
): Date => {
  let remaining = mouseX;
      ...
      while (weekStart <= lastWeekStart) {
        ...
        if (remaining <= columnWidth) {
          const dayOffset = Math.min(
            6,
            Math.floor(remaining / (columnWidth / 7))
          );
...
const getDifferenceIn = (range: Range) => {
  ...
  if (range === "weekly") {
    fn = (dateLeft, dateRight) =>
      differenceInWeeks(
        startOfWeek(dateLeft, weekStartsOn),
        startOfWeek(dateRight, weekStartsOn)
      );
...
const getStartOf = (range: Range) => {
  ...
  if (range === "weekly") {
    fn = (date) => startOfWeek(date, weekStartsOn);
  } else if (range === "monthly" || range === "quarterly") {
    fn = startOfMonth;
  }
...
const getDateByMousePosition = (context: GanttContextProps, mouseX: number) => {
  ...
  if (context.range === "weekly") {
    return getDateFromWeeklyHeaderColumn(
      mouseX,
      context.timelineData,
      timelineStartDate,
      columnWidth
...
const getOffset = (
  ...
  if (context.range === "weekly") {
    const headerOffset = getWeeklyHeaderColumnOffset(
      date,
      context.timelineData,
      timelineStartDate
    );
    ...
    if (headerOffset) {
      return (
        headerOffset.weekColumns * parsedColumnWidth +
        headerOffset.dayInWeek * pixelsPerDay
      );
    }

    const partialColumns = differenceInDays(
      startOfDay(date),
...
const getWidth = (
  ...
  if (context.range === "weekly") {
    const pixelsPerDay = parsedColumnWidth / 7;
    const startPx = getOffset(startAt, timelineStartDate, context);
    const endPx = getOffset(addDays(startOfDay(endAt), 1), timelineStartDate, context);

    return Math.max(endPx - startPx, pixelsPerDay);
  }
...
const WeeklyHeader: FC = () => {
  ...
  return gantt.timelineData.map((year) =>
    year.quarters
      .flatMap((quarter) => quarter.months)
      .map((_month, index) => {
        const columnCount = weeksInMonth(year.year, index);
        ...
        return (
          ...
          >
            <GanttContentHeader
              columns={columnCount}
              renderHeaderItem={(item: number) => {
                const weekStart = weekStartInMonth(year.year, index, item);
                return (
                  <div className="flex flex-col items-center gap-0.5 leading-tight">
                    <p className="font-medium">W{getISOWeek(weekStart)}</p>
              ...
              title={format(new Date(year.year, index, 1), "MMMM yyyy")}
            />
            <GanttColumns columns={columnCount} />
          </div>
        );
      })
  );
...
export type GanttSidebarItemProps = {
  ...
  /** Match timeline height when overlapping events use multiple sub-rows. */
  rowCount?: number;
  /** Draw a subtle line above this sequence row. */
  ...
  return (
    <div
      className={cn(
        ...
        showTopDivider && "gantt-sequence-divider-top",
        className
      )}
      key={feature.id}
      onClick={handleClick}
      ...
      role="button"
      style={{
        height: `calc(var(--gantt-row-height) * ${Math.max(1, rowCount)})`,
      }}
      tabIndex={0}
    >
...
export const GanttSidebarGroup: FC<GanttSidebarGroupProps> = ({
  ...
  <div className={className}>
    <p
      className="gantt-group-divider-bottom w-full truncate p-2.5 text-left font-medium text-muted-foreground text-xs"
...
export const GanttColumn: FC<GanttColumnProps> = ({
  ...
  const top = useThrottle(pointerY, 10);
...
export const GanttCreateMarkerTrigger: FC<GanttCreateMarkerTriggerProps> = ({
  ...
  const x = useThrottle(pointerX, 10);
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    <Card
      className="h-full w-full rounded-md border-0 bg-transparent p-0 text-xs shadow-sm ring-0"
      style={{
    ...
    >
      <div
        className={cn(
          "flex h-full w-full items-center px-2 text-left",
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({
  ...
  const timelineStartDate = useMemo(
    () => getTimelineStartDate(gantt.timelineData, gantt.range),
...
export const GanttFeatureListGroup: FC<GanttFeatureListGroupProps> = ({
  ...
  <div className={className}>
    <div
      aria-hidden
      className="gantt-group-divider-bottom"
      style={{ height: "var(--gantt-row-height)" }}
    />
    {children}
  </div>
...
export const GanttFeatureRow: FC<GanttFeatureRowProps> = ({
  ...
  const maxSubRows = computeSequenceSubRowCount(sortedFeatures);
  const subRowHeight = 36; // Base row height — keep in sync with --gantt-row-height
...
export const GanttFeatureList: FC<GanttFeatureListProps> = ({
  ...
  <div
    className={cn("absolute inset-x-0 top-0 h-full w-full space-y-4", className)}
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  const scrollToFeature = useCallback(
    (feature: GanttFeature) => {
      ...
      const timelineStartDate = getTimelineStartDate(timelineData, range);
